...
class TestAsyncSegments:
    ...
    async def test_method_retrieve(self, client: AsyncIntercom) -> None:
        segment = await client.segments.retrieve(
            "string",
        )
        assert_matches_type(Segment, segment, path=["response"])
    ...
    async def test_raw_response_retrieve(self, client: AsyncIntercom) -> None:
        response = await client.segments.with_raw_response.retrieve(
            "string",
        )
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        segment = response.parse()
    ...
    async def test_method_list(self, client: AsyncIntercom) -> None:
        segment = await client.segments.list()
        assert_matches_type(SegmentList, segment, path=["response"])
    ...
    async def test_raw_response_list(self, client: AsyncIntercom) -> None:
        response = await client.segments.with_raw_response.list()
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        segment = response.parse()
        assert_matches_type(SegmentList, segment, path=["response"])
